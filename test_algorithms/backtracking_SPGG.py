...
def execute_backtracking_spgg():
    print("Algorihtm: backtracking")
    start_time = clock.time()

    Sensors_Data = pd.read_json('database_data/SPGG_readings.json')
    Aircon_Data = pd.read_json('database_data/SPGG_aircon_status.json')
    ...
    humidity_col = [
        col for col in merged_data.columns 
        if "humidity" in col.lower() and "24e124136d336145" not in col.lower()
    ]
    door_col = [
        col for col in merged_data.columns
    ...
    for i in range(total_final_rows - 1):
        ...
        curr_door_status = final_data["door_status"].iloc[i]
    ...
    aircon_status_result = aircon_status_result[aircon_status_result["door_status"] == 1]
    aircon_status_result = aircon_status_result[aircon_status_result["outdoor_temp"] > 24.5]
    print('outdoor')


    
    aircon_status_result = aircon_status_result.sort_values(by=['current_temp'], ascending=False) 
    aircon_status_result.to_csv('saved_data/aircon_status_SPGG_backtracking.csv', index=False)
    # aircon_status_result.info()

    aircon_status_getBestSettings_result = aircon_status_getBestSettings_result[aircon_status_getBestSettings_result["door_status"] == 1]
    aircon_status_getBestSettings_result = aircon_status_getBestSettings_result.sort_values(by=['current_temp'], ascending=False)
    aircon_status_getBestSettings_result.to_csv("saved_data/aircon_status_SPGG_getBestSettings.csv", index=False)
    ...
    with open('saved_data/stored_dictionary_backtracking_spgg.json', 'w') as f:
        json.dump(stored_dictionary, f, default=str, indent=4)

    print("successful json conversion")
    print(f"data Preparation and stored dictionary successful. Time taken (backtracking): {elapsed_time_minutes:.2f} minutes with {acceptable_range} acceptable range")
